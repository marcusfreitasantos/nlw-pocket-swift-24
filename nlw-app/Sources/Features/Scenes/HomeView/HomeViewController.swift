...
class HomeViewController: UIViewController {
    ...
    override func viewDidLoad(){
        ...
        homeViewModel.fetchInitialData{ [weak self] categories in
                guard let self = self else {return}
                    self.homeView.updateFilterButtons(with: categories) {
                        selectedCategory in self.filterPlaces(by: selectedCategory)
                    }
        }
    ...
    private func addAnnotationsToMap(){
        ...
        if let firstAnnotation = annotations.first {
            homeView.mapView.setRegion(MKCoordinateRegion(center: firstAnnotation.coordinate, span:
...
extension HomeViewController: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, annotationView view: MKAnnotationView, callloutAccessoryControlTapped control: UIControl){
